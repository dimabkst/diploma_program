...
class boundary_conditions_input:
    ...
    def __init__(self, root):
        ...
        # boundary conditions
        self.boundary_top_frame = ttk.Frame(self.boundary_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.boundary_top_frame.grid(column=0, row=0, sticky=(N, W, E, S))

        self.boundary_bot_frame = ttk.Frame(self.boundary_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.boundary_bot_frame.grid(column=0, row=1, sticky=(N, W, E, S))

        self.boundary_top_left_frame = ttk.Frame(self.boundary_top_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.boundary_top_left_frame.grid(column=0, row=0, sticky=(N, W, E, S))

        self.boundary_top_right_frame = ttk.Frame(self.boundary_top_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.boundary_top_right_frame.grid(column=1, row=0, sticky=(N, W, E, S))

        # RG input
        self.RG_frame = ttk.Frame(self.boundary_top_left_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.RG_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.RG_label_frame = ttk.Frame(self.RG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.RG_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.RG_entry_frame = ttk.Frame(self.RG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.RG_entry_frame.grid(column=1, row=0, sticky=(N, W, E, S))

        self.RG_var = StringVar()
        self.RG_var.set("1")
        self.RG_var.trace("w", lambda name, index, mode: self.change_and_show_boundary())
        ...
        self.count_entry = ttk.Entry(self.RG_entry_frame, width=ENTRY_WIDTH, textvariable=self.RG_var)
        self.count_entry.grid(column=0, row=0, sticky=(N, E, W, S))
        #

        # LrG input
        self.LrG_frame = ttk.Frame(self.boundary_top_left_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.LrG_frame.grid(column=0, row=1, sticky=(N, W, E, S))
        self.LrG_label_frame = ttk.Frame(self.LrG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.LrG_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.LrG_LrG_frame = ttk.Frame(self.LrG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        ...
        for i in range(int(self.RG_var.get() or 0)):
            ...
            self.LrG_vars[i].trace("w", lambda name, index, mode: self.change_and_show_boundary())

            self.LrG_labels.append(ttk.Label(self.LrG_LrG_frame, text=f"L{i + 1}G(dx):", style="WhiteBg.TLabel"))
            self.LrG_labels[i].grid(row=i, column=0, sticky=(N, W, E, S))

            self.LrG_entries.append(ttk.Entry(self.LrG_LrG_frame, width=ENTRY_WIDTH, textvariable=self.LrG_vars[i]))
        ...
        # LG input
        self.LG_frame = ttk.Frame(self.boundary_top_right_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.LG_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.LG_label_frame = ttk.Frame(self.LG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.LG_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.LG_entry_frame = ttk.Frame(self.LG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.LG_entry_frame.grid(column=1, row=0, sticky=(N, W, E, S))

        self.LG_var = StringVar()
        self.LG_var.set("1")
        self.LG_var.trace("w", lambda name, index, mode: self.change_and_show_boundary())
        ...
        self.count_entry = ttk.Entry(self.LG_entry_frame, width=ENTRY_WIDTH, textvariable=self.LG_var)
        self.count_entry.grid(column=0, row=0, sticky=(N, E, W, S))
        #

        # slG input
        self.slG_frame = ttk.Frame(self.boundary_top_right_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.slG_frame.grid(column=0, row=1, sticky=(N, W, E, S))
        self.slG_label_frame = ttk.Frame(self.slG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.slG_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.slG_slG_frame = ttk.Frame(self.slG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        ...
        # YrlG input
        self.yrlG_frame = ttk.Frame(self.boundary_bot_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.yrlG_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.yrlG_label_frame = ttk.Frame(self.yrlG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.yrlG_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.yrlG_yrlG_frame = ttk.Frame(self.yrlG_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        ...
        for r in range(int(self.RG_var.get() or 0)):
            ...
            for l in range(int(self.LG_var.get() or 0)):
                ...
                self.yrlG_labels[r][l].grid(row=r, column=l * 2, sticky=(N, W, E, S))

                self.yrlG_vars[r].append(StringVar())
                self.yrlG_vars[r][l].set("0")
                self.yrlG_vars[r][l].trace("w", lambda name, index, mode: self.change_and_show_boundary())

                self.yrlG_entries[r].append(ttk.Entry(self.yrlG_yrlG_frame, width=ENTRY_WIDTH,
                                                      textvariable=self.yrlG_vars[r][l]))
                self.yrlG_entries[r][l].grid(row=r, column=l * 2 + 1, sticky=(N, W, E, S))
        #

        #

        # Align everything
        self.align_rows_cols(self.RG_label_frame)
        self.align_rows_cols(self.RG_entry_frame)
        self.align_rows_cols(self.RG_frame)

        self.align_rows_cols(self.LrG_frame)
        self.align_rows_cols(self.LrG_label_frame)
        self.align_rows_cols(self.LrG_LrG_frame)

        self.align_rows_cols(self.LG_label_frame)
        self.align_rows_cols(self.LG_entry_frame)
        self.align_rows_cols(self.LG_frame)

        self.align_rows_cols(self.slG_frame)
        self.align_rows_cols(self.slG_label_frame)
        self.align_rows_cols(self.slG_slG_frame)

        self.align_rows_cols(self.boundary_top_left_frame)
        self.align_rows_cols(self.boundary_top_right_frame)
        self.align_rows_cols(self.boundary_top_frame)

        self.align_rows_cols(self.yrlG_frame)
        self.align_rows_cols(self.yrlG_label_frame)
        self.align_rows_cols(self.yrlG_yrlG_frame)

        self.align_rows_cols(self.boundary_bot_frame)
                ...
                for i in range(max(old_RG, int(self.RG_var.get() or 0))):
                    if i >= min(old_RG, int(self.RG_var.get() or 0)):
                        if old_RG > int(self.RG_var.get() or 0):
                            ...
                            self.yrlG_vars.append([StringVar() for _ in range(int(self.LG_var.get() or 0))])
                            ...
                            for k in range(int(self.LG_var.get() or 0)):
                                ...
                                self.yrlG_labels[i][k].grid(row=i, column=k * 2, sticky=(N, W, E, S))
                                self.yrlG_entries[i][k].grid(row=i, column=k * 2 + 1, sticky=(N, W, E, S))
                    ...
                    for j in range(max(old_LG, int(self.LG_var.get() or 0))):
                        if j >= min(old_LG, int(self.LG_var.get() or 0)):
                            if old_LG > int(self.LG_var.get() or 0):
                                ...
                                self.yrlG_entries[i][j].grid(row=i, column=j * 2 + 1, sticky=(N, W, E, S))
                        else:
