...
class desired_conditions_input:
    ...
    def __init__(self, root):
        ...
        # desired conditions
        self.desired_top_frame = ttk.Frame(self.desired_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.desired_top_frame.grid(column=0, row=0, sticky=(N, W, E, S))

        self.desired_bot_frame = ttk.Frame(self.desired_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.desired_bot_frame.grid(column=0, row=1, sticky=(N, W, E, S))

        self.desired_top_left_frame = ttk.Frame(self.desired_top_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.desired_top_left_frame.grid(column=0, row=0, sticky=(N, W, E, S))

        self.desired_top_right_frame = ttk.Frame(self.desired_top_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.desired_top_right_frame.grid(column=1, row=0, sticky=(N, W, E, S))

        # I input
        self.I_frame = ttk.Frame(self.desired_top_left_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.I_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.I_label_frame = ttk.Frame(self.I_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.I_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.I_entry_frame = ttk.Frame(self.I_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.I_entry_frame.grid(column=1, row=0, sticky=(N, W, E, S))

        self.I_var = StringVar()
        self.I_var.set("1")
        self.I_var.trace("w", lambda name, index, mode: self.change_and_show_desired())

        ttk.Label(self.I_label_frame, text="Кількість операторів Li(dx, dt) -", style="WhiteBg.TLabel") \
            .grid(column=0, row=0, sticky=(N, E, W, S))

        self.count_entry = ttk.Entry(self.I_entry_frame, width=ENTRY_WIDTH, textvariable=self.I_var)
        self.count_entry.grid(column=0, row=0, sticky=(N, E, W, S))
        #

        # Li input
        self.Li_frame = ttk.Frame(self.desired_top_left_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.Li_frame.grid(column=0, row=1, sticky=(N, W, E, S))
        self.Li_label_frame = ttk.Frame(self.Li_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.Li_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.Li_Li_frame = ttk.Frame(self.Li_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        ...
        for i in range(int(self.I_var.get() or 0)):
            ...
            self.Li_vars[i].trace("w", lambda name, index, mode: self.change_and_show_desired())

            self.Li_labels.append(ttk.Label(self.Li_Li_frame, text=f"L{i + 1}(dx, dt):", style="WhiteBg.TLabel"))
            self.Li_labels[i].grid(row=i, column=0, sticky=(N, W, E, S))

            self.Li_entries.append(ttk.Entry(self.Li_Li_frame, width=ENTRY_WIDTH, textvariable=self.Li_vars[i]))
        ...
        # Ji input
        self.Ji_frame = ttk.Frame(self.desired_top_right_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.Ji_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.Ji_label_frame = ttk.Frame(self.Ji_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.Ji_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.Ji_Ji_frame = ttk.Frame(self.Ji_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        ...
        for i in range(int(self.I_var.get() or 0)):
            ...
            self.Ji_vars[i].trace("w", lambda name, index, mode: self.change_and_show_desired())
            ...
            self.Ji_entries.append(ttk.Entry(self.Ji_Ji_frame, width=ENTRY_WIDTH, textvariable=self.Ji_vars[i]))
            self.Ji_entries[i].grid(row=1, column=i, sticky=(N, W, E, S))
        #

        # sij input
        self.sij_frame = ttk.Frame(self.desired_top_right_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.sij_frame.grid(column=0, row=1, sticky=(N, W, E, S))
        self.sij_label_frame = ttk.Frame(self.sij_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.sij_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.sij_sij_frame = ttk.Frame(self.sij_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        ...
        sij_format_image_label = ttk.Label(self.sij_label_frame, image=sij_format_image, style="WhiteBg.TLabel")
        ...
        for i in range(int(self.I_var.get() or 0)):
            ...
            for j in range(int(self.Ji_vars[i].get() or 0)):
                self.sij_labels[i].append(ttk.Label(self.sij_sij_frame, text=f"s{i + 1}{j + 1}", style="WhiteBg.TLabel"))
                self.sij_labels[i][j].grid(row=i, column=j * 2, sticky=(N, W, E, S))

                self.sij_vars[i].append(StringVar())
                self.sij_vars[i][j].set("(0,0)")
                self.sij_vars[i][j].trace("w", lambda name, index, mode: self.change_and_show_desired())

                self.sij_entries[i].append(ttk.Entry(self.sij_sij_frame, width=ENTRY_WIDTH, textvariable=self.sij_vars[i][j]))
                self.sij_entries[i][j].grid(row=i, column=j * 2 + 1, sticky=(N, W, E, S))
        #

        # Yij input
        self.yij_frame = ttk.Frame(self.desired_bot_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.yij_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.yij_label_frame = ttk.Frame(self.yij_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        self.yij_label_frame.grid(column=0, row=0, sticky=(N, W, E, S))
        self.yij_yij_frame = ttk.Frame(self.yij_frame, padding="3 3 12 12", style="WhiteBg.TFrame")
        ...
        for i in range(int(self.I_var.get() or 0)):
            ...
            for j in range(int(self.Ji_vars[i].get() or 0)):
                ...
                self.yij_labels[i][j].grid(row=i, column=j * 2, sticky=(N, W, E, S))
                ...
                self.yij_entries[i][j].grid(row=i, column= j * 2 + 1, sticky=(N, W, E, S))
        #
                ...
                for i in range(max(old_I, int(self.I_var.get() or 0))):
                    ...
                    if i >= min(old_I, int(self.I_var.get() or 0)):
                        if old_I > int(self.I_var.get() or 0):
                            ...
                            self.sij_vars.append([StringVar() for _ in range(int(self.Ji_vars[i].get() or 0))])
                            ...
                            for k in range(int(self.Ji_vars[i].get() or 0)):
                                ...
                                self.sij_vars[i][k].trace("w", lambda name, index, mode: self.change_and_show_desired())
                                self.sij_labels[i][k].grid(row=i, column=k * 2, sticky=(N, W, E, S))
                                self.sij_entries[i][k].grid(row=i, column=k * 2 + 1, sticky=(N, W, E, S))
                    ...
                    for j in range(max(old_Ji, int(self.Ji_vars[i].get() or 0))):
                        if j >= min(old_Ji, int(self.Ji_vars[i].get() or 0)):
                            ...
                            self.sij_labels[i][j] = ttk.Label(self.sij_sij_frame, text=f"s{i + 1}{j + 1}", style="WhiteBg.TLabel")
                            self.sij_labels[i][j].grid(row=i, column=j * 2, sticky=(N, W, E, S))
                ...
                for i in range(max(old_I, int(self.I_var.get() or 0))):
                    ...
                    if i >= min(old_I, int(self.I_var.get() or 0)):
                        if old_I > int(self.I_var.get() or 0):
                            ...
                            self.yij_vars.append([StringVar() for _ in range(int(self.Ji_vars[i].get() or 0))])
                            ...
                            for k in range(int(self.Ji_vars[i].get() or 0)):
                                ...
                                self.yij_labels[i][k].grid(row=i, column=k * 2, sticky=(N, W, E, S))
                                self.yij_entries[i][k].grid(row=i, column=k * 2 + 1, sticky=(N, W, E, S))
